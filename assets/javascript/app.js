...
const pages = ['index.htm', 'about_us.htm', 'legislation.htm'];

// Define a function to load the content of a page
...
function testAboutUsLinkHref() {
  // Load each page and check the href attribute of the Pricing link
  pages.forEach(page => {
    const pageContent = loadPage(page);
    const parser = new DOMParser();
    const doc = parser.parseFromString(pageContent, 'text/html');
    const elements = doc.querySelectorAll('.nav-link');
    let allElementsHaveCorrectHref = true;
    elements.forEach(element => {
      if (element.innerHTML === 'About Us' && element.getAttribute('href') !== 'about_us.htm') {
        allElementsHaveCorrectHref = false;
      }
    });
    if (allElementsHaveCorrectHref) {
      console.log(`${page}: All elements have the correct href attribute value.`);
    } else {
      console.log(`${page}: Not all elements have the correct href attribute value.`);
    }
  });
}
